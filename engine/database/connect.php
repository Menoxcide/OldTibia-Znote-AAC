...
//Decide which DB to use depending on world
if (isset($_GET['world']) && $_GET['world'] == $configworlds[2]){ //Check if TEST world
	$connect = new mysqli($config['sqlHost'], $config['sqlUser'], $config['sqlPassword'], $config['sqlDatabaseTest']);
...
//Manually connect to first database
$connectFirst = new mysqli($config['sqlHost'], $config['sqlUser'], $config['sqlPassword'], $config['sqlDatabase']);
if ($connectFirst->connect_errno) {
    die("Failed to connect to MySQL: (" . $connectFirst->connect_errno . ") " . $connectFirst->connect_error . $install);
}
//Manually connect to test database
$connectTest = new mysqli($config['sqlHost'], $config['sqlUser'], $config['sqlPassword'], $config['sqlDatabaseTest']);
if ($connectTest->connect_errno) {
    die("Failed to connect to MySQL: (" . $connectTest->connect_errno . ") " . $connectTest->connect_error . $install);
}
...
// mysql insert second server
function mysql_insert_test($query){ voidQueryTest($query); }
// mysql delete
...
//////// Second World Functions ////////
// Select single row from first database
function mysql_select_single_first($query) {
  global $connectFirst;
  global $aacQueries;
  $aacQueries++;
  ...
  global $accQueriesData;
  $accQueriesData[] = $query;
  $result = mysqli_query($connectFirst,$query) or die(var_dump($query)."<br>(query - <font color='red'>SQL error</font>) <br>Type: <b>select_single</b> (select single row from database)<br><br>".mysqli_error($connectFirst));
  $row = mysqli_fetch_assoc($result);
  return !empty($row) ? $row : false;
} 
// Select single row from second database
function mysql_select_single_test($query) {
  global $connectTest;
  global $aacQueries;
  $aacQueries++;
  ...
  global $accQueriesData;
  $accQueriesData[] = $query;
  $result = mysqli_query($connectTest,$query) or die(var_dump($query)."<br>(query - <font color='red'>SQL error</font>) <br>Type: <b>select_single</b> (select single row from database)<br><br>".mysqli_error($connectTest));
  $row = mysqli_fetch_assoc($result);
  return !empty($row) ? $row : false;
} 
// Void Query for First database.
function voidQueryFirst($query) {
  global $connectFirst;
  global $aacQueries;
  $aacQueries++;
  global $accQueriesData;
  $accQueriesData[] = $query;
  mysqli_query($connectFirst,$query) or die(var_dump($query)."<br>(query - <font color='red'>SQL error</font>) <br>Type: <b>voidQuery</b> (voidQuery is used for update, insert or delete from database)<br><br>".mysqli_error($connectFirst));
}

// Void Query for second database.
function voidQueryTest($query) {
  global $connectTest;
  global $aacQueries;
  $aacQueries++;
  global $accQueriesData;
  $accQueriesData[] = $query;
  mysqli_query($connectTest,$query) or die(var_dump($query)."<br>(query - <font color='red'>SQL error</font>) <br>Type: <b>voidQuery</b> (voidQuery is used for update, insert or delete from database)<br><br>".mysqli_error($connectTest));
}
