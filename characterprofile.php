...
<table class="heading">
    <tbody>
        <tr class="transparent nopadding">
            ...
            <td valign="middle">
			Information about all players on OldTibia.
            </td>
        </tr>
    </tbody>
</table>
...
if (isset($_GET['name']) === true && empty($_GET['name']) === false) {
	...
	if ($user_id !== false) {
		if ($config['TFSVersion'] == 'TFS_10') {
			$profile_data = user_character_data($user_id, 'name', 'level', 'vocation', 'lastlogin', 'sex', 'group_id', 'town_id', 'world');
			$profile_data['online'] = user_is_online_10($user_id);
		} else $profile_data = user_character_data($user_id, 'name', 'level', 'vocation', 'lastlogin', 'online', 'sex', 'group_id', 'town_id', 'world');
		...
		<!-- PROFILE MARKUP HERE-->
			...
			<tr><td>World:</td>
			<td><?php echo world_id_to_name($profile_data['world']); ?></td></tr>
			<tr><td>Residence:</td>
			<td><?php 
						...
						if ($array) { ?>
							...
							<table>
								...
								foreach ($deaths as $d) {
										...
										$lasthit = ($d['is_player']) ? "<a href='characterprofile.php?name=".$d['killed_by']."&world='. $world .''>".$d['killed_by']."</a>" : $d['killed_by'];
											if ($d['is_player'] > 0) {
										...
										if ($mostdmg) {
											$mostdmg = ($d['mostdamage_is_player']) ? "<a href='characterprofile.php?name=".$d['mostdamage_by']."&world='. $world .''>".$d['mostdamage_by']."</a>" : $d['mostdamage_by'];
											echo " and by $mostdmg.";
								...
								foreach ($array as $value) {
									...
									if ($value[3] !== false && $value[3] >= 1) {
										...
										if ($namedata !== false) {
											...
											$value[3] = 'player: <a href="characterprofile.php?name='. $value[3] .'&world='. $world .'">'. $value[3] .'</a>';
										} else {
							...
							?>
								...
								foreach ($characters as $char) {
										if (hide_char_to_name(user_character_hide($char['name'])) != 'hidden') {
											echo '<tr>';
											echo '<td>'. $number .'. <a href="characterprofile.php?name='. $char['name'] .'&world='. $world .'">'. $char['name'] .'</a></td>';
		...
		echo htmlentities(strip_tags($name, ENT_QUOTES)).' does not exist on this world.';
	}
} else {
