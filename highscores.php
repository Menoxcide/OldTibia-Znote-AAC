...
if ($scores) {
	...
	<table class="heading">
		<tbody>
            ...
            <td valign="middle">
			Browse the highest ranked players on OldTibia
            </td>
        </tr>
    </tbody>
</table>
	...
	<form action="" method="GET">
		...
		<input type="submit" value=" Browse " class="btn btn-info">
	</form>
	<table id="highscoresTable" class="table table-striped table-hover">
		...
		for ($i = 0; $i < count($scores[$type]); $i++) {
			if (pageCheck($i, $page, $rowsPerPage)) {
				...
				<tr>
					<td><?php echo $i+1; ?></td>
					<td><a href="characterprofile.php?name=<?php echo $scores[$type][$i]['name']; ?>&world=<?php echo $getworldname;?>"><?php echo $scores[$type][$i]['name']; ?></a></td>
