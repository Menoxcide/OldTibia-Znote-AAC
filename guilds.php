...
<table class="heading">
    <tbody>
        <tr class="transparent nopadding">
            ...
            <td valign="middle">
			Information about all guilds on OldTibia.
            </td>
        </tr>
    </tbody>
</table>
...
if ($config['TFSVersion'] != 'TFS_10') $guilds = mysql_select_multi("SELECT `t`.`id`, `t`.`name`, `t`.`creationdate`, (SELECT count(p.rank_id) FROM players AS p LEFT JOIN guild_ranks AS gr ON gr.id = p.rank_id WHERE gr.guild_id =`t`.`id`) AS `total` FROM `guilds` as `t` ORDER BY `t`.`name`;");
else $guilds = mysql_select_multi("SELECT `id`, `name`, `creationdate`, (SELECT COUNT('guild_id') FROM `guild_membership` WHERE `guild_id`=`id`) AS `total` FROM `guilds` ORDER BY `name`;");
	...
	<tr class="yellow">
		...
		foreach ($guilds as $guild) {
			if ($guild['total'] >= 1) {
				...
				echo '<td>'. getClock($guild['creationdate'], true) .'</td>';
				echo '</tr>';
			}
		}
		?>
		...
		if (user_character_account_id($_POST['selected_char']) === $session_user_id) {
			...
			if ($char_data['level'] >= $config['create_guild_level']) {
				...
				if ($char_data['online'] == 0) {
					$acc_data = user_data($user_data['id'], 'premend');
					
					// If character is premium
					if ($config['guild_require_premium'] == false || $acc_data['premend'] > 0) {
...
if ($highest_access >= 2) {
	...
	if ($config['TFSVersion'] == 'OTH' || $config['TFSVersion'] == 'TFS_10' && $config['guildwar_enabled'] === true) {
	...
	?>
		...
		<?php } ?>
		<?php if ($config['TFSVersion'] == 'TFS_10' && $config['guildwar_enabled'] === true) { ?>
...
if ($config['guildwar_enabled'] === true) {
	if ($config['TFSVersion'] == 'OTH' || $config['TFSVersion'] == 'TFS_10') $wardata = get_guild_wars();
	else if ($config['TFSVersion'] == 'TFS_03') $wardata = get_guild_wars03();
	else die("Can't recognize TFS version. It has to be either OTH or TFS_03. Correct this in config.php");
	$war_exist = false;
	if ($wardata !== false) {
