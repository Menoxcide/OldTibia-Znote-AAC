...
function guild_change_leader($nCid, $oCid) {
        ...
        if ($status) {
                ...
                // Update guilds set new owner_id
...
// END GUILD WAR
// ADMIN FUNCTIONS
function set_ingame_position02($name, $acctype) {
        $acctype = (int)$acctype;
        $name = sanitize($name);
...
function user_character_list($account_id) {
        ...
        if (config('TFSVersion') == 'TFS_10') {
                $characters = mysql_select_multi("SELECT `id`, `name`, `level`, `vocation`, `town_id`, `lastlogin`, `world` FROM `players` WHERE `account_id`='$account_id' ORDER BY `level` DESC");
                if ($characters !== false) {
        ...
        } else $characters = mysql_select_multi("SELECT `name`, `level`, `vocation`, `town_id`, `lastlogin`, `online`, `world` FROM `players` WHERE `account_id`='$account_id' ORDER BY `level` DESC");
                ...
                if ($player_account_id == $account_id) {
                        ...
                        if ($ip == $ip_data['ip']) {
                                ...
                                if ($mode == 'username') {
                                        ...
                                        echo '<br><p>Your username is:</p> <h3>'. $name_data['id'] .'</h3>';
                                } else {
...
function user_account_add_premdays($accid, $days) {
    $accid = (int)$accid;
    $days = (int)$days;
    $tmp = mysql_result(mysql_select_single("SELECT `premend` FROM `accounts` WHERE `id`='$accid';"), 0, 'premend');
    if($tmp == 0)
    {
...
function user_create_account($register_data) {
		...
		mysql_insert_test("INSERT INTO `accounts` ($fields) VALUES ($data)");
       
        $account_id = user_id($register_data['id']);
        mysql_insert("INSERT INTO `znote_accounts` (`account_id`, `ip`, `created`) VALUES ('$account_id', '$ip', '$created')");
		mysql_insert_test("INSERT INTO `znote_accounts` (`account_id`, `ip`, `created`) VALUES ('$account_id', '$ip', '$created')");
...
function user_create_character($character_data) {
        ...
        echo 1;
               //If world=2 create character on Test Server
                if ($character_data['world'] == 1) {
                        mysql_insert("INSERT INTO `players`(`$fields_sql`) VALUES ('$data_sql');");
                }else {
                        mysql_insert_test("INSERT INTO `players`(`$fields_sql`) VALUES ('$data_sql');");
                }
        ...
        echo 2;
		        if ($character_data['world'] == 1) {
						...
						$charid = user_character_id_test($import_data['name']);
						mysql_insert_test("INSERT INTO `znote_players`(`player_id`, `created`, `hide_char`, `comment`) VALUES ('$charid', '$created', '0', '');");
                }
}
...
function user_premend($user_id) {
        $data = array();
        $user_id = sanitize($user_id);
       
        return mysql_select_single("SELECT `premend` FROM `accounts` WHERE `id` = $user_id;");
...
function user_login_id_03($username, $password) {
        if (config('salt') === true) {
                if (user_exist($username)) {
                        ...
                        $data = mysql_select_single("SELECT `salt`, `id`, `password` FROM `accounts` WHERE `id`='$user_id';");
                        $salt = $data['salt'];
                        if (!empty($salt)) $password = sha1($salt.$password);
                        else $password = sha1($password);
                        return ($data !== false && $data['id'] == $username && $data['password'] == $password) ? $data['id'] : false;
                } else return false;
...
// Get WORLD ID from character name
function user_world_id($charname) {
        $charname = sanitize($charname);
        $char = mysql_select_single("SELECT `world` FROM `players` WHERE `name`='$charname';");
        if ($char !== false) return $char['world'];
        else return false;
}
// Get character ID from character name on TEST Server
 function user_character_id_test($charname) {
        $charname = sanitize($charname);
        $char = mysql_select_single_test("SELECT `id` FROM `players` WHERE `name`='$charname';");
        if ($char !== false) return $char['id'];
        else return false;
}
...
function user_login_03($username, $password) {
        if (config('salt') === true) {
                $username = sanitize($username);
                $data = mysql_select_single("SELECT `salt`, `id`, `password` FROM `accounts` WHERE `id`='$username';");
                $salt = $data['salt'];
                if (!empty($salt)) $password = sha1($salt.$password);
                else $password = sha1($password);
                return ($data !== false && $data['id'] == $username && $data['password'] == $password) ? $data['id'] : false;
