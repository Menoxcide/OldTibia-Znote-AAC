...
include 'layout/overall/header.php'; 
		...
		function selectcolor($value)
		{
			if($GLOBALS['action'] == $value)
				return '#303030; color: #FFFFFF';
			else
				return '#159150; color: #aaaaaa';
		}
...
if (!empty($_POST['selected_comment'])) {
	...
	date_default_timezone_set('EST');//or change to whatever timezone you want
	...
	if($pending_delete)
		foreach($pending_delete as $delete) {
			if(new DateTime($delete['time']) > new DateTime())
				echo '<b>CAUTION!</b> Your character with name <b>' . $delete['character_name'] . ' will be deleted on ' . $delete['time'] . '</b>. <a href="myaccount.php?cancel_delete_id=' . $delete['id'] . '&world=' . $world . '">Cancel this operation.</a><br/>';
			else {
	...
	<table class="heading">
		<tbody>
			<tr class="transparent nopadding">
				...
				<td valign="middle">
					Your account page and information, dont tell anyone your details.
				</td>
			</tr>
		</tbody>
	</table>
		<p>Welcome to your account page, logged in with account number <b><?php echo $user_data['id']; ?></b>. &nbsp;&nbsp;&nbsp;&nbsp; <a href="changepassword.php"><input class="button" type="submit" value="Change Password"></a> &nbsp;&nbsp; <a href="settings.php"><input class="button" type="submit" value="Settings"></a> &nbsp;&nbsp; <a href="logout.php"><input class="button" type="submit" value="Logout"></a><br>
		<?php 			
		if ($user_data['premend'] != 0) {
				...
				echo 'You do not have premium account days. <a href="buypoints.php"><input class="button" type="submit" value="Buy Premium"></a>';
			}
		?></p>
	<table class="heading">
		<tbody>
			<tr class="transparent nopadding">
				...
				<td valign="middle">
					<?php echo $char_count; ?> characters are on this account. &nbsp;&nbsp;&nbsp; <a href="createcharacter.php"><input class="button" type="submit" value="Create Character"></a>
				</td>
			</tr>
		</tbody>
	</table>		
	
	<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=4><TR><TD ALIGN=left CLASS=white colspan="2"><B>Select a world: </B>
		<a href="?world=<?php echo $configworlds[1] ?>" style="padding: 5px 5px 7px 5px; margin: 5px 1px 0px 1px;"><?php echo $configworlds[1] ?></a>
		<a href="?world=<?php echo $configworlds[2] ?>" style="padding: 5px 5px 7px 5px; margin: 5px 1px 0px 1px;"><?php echo $configworlds[2] ?></a></tr>
	</TD></TR></TD></TR></table>
		...
		if ($char_array) {
			...
			<table id="myaccountTable" class="table table-striped table-hover">
				<tr class="yellow">
					<th style="background-color:#505050"><font color="white"><strong>NAME</strong></font></th><th style="background-color:#505050"><font color="white"><strong>LEVEL</strong></font></th><th style="background-color:#505050"><font color="white"><strong>VOCATION</strong></font></th><th style="background-color:#505050"><font color="white"><strong>TOWN</strong></font></th><th style="background-color:#505050"><font color="white"><strong>LAST LOGIN</strong></font></th><th style="background-color:#505050"><font color="white"><strong>STATUS</strong></font></th><th style="background-color:#505050"><font color="white"><strong>HIDE</strong></font></th>
				</tr>
				<?php
				$characters = array();
				foreach ($char_array as $value) {
					...
					echo '<tr>';
					echo '<td><a href="characterprofile.php?name='. $value['name'] .'&world='. $getworldname .'">'. $value['name'] .'</a></td><td>'. $value['level'] .'</td><td>'. $value['vocation'] .'</td><td>'. $value['town_id'] .'</td><td>'. $value['lastlogin'] .'</td><td>'. $value['online'] .'</td><td>'. hide_char_to_name(user_character_hide($value['name'])) .'</td>';
					echo '</tr>';
	...
	<table class="heading">
		<tbody>
			<tr class="transparent nopadding">
				...
				<td valign="middle">
					Adjust the settings of your characters.
				</td>
			</tr>
		</tbody>
	</table>	
			...
			<form action="" method="post">
						...
						<?php
							...
							Token::create();
						?>
						<input type="submit" class="button" value="Toggle hide" class="btn btn-info">
			</form></td>
			...
			<td><form action="" method="post">
						...
						<?php
							...
							Token::create();
						?>
						<input type="submit" class="button" value="Change comment" class="btn btn-info">
			</form></td>
			...
			<td><form action="" method="post">
						...
						<?php
							...
							Token::create();
						?>
						<input type="submit" class="button" value="Change gender" class="btn btn-info">
			</form></td>
			...
			<td><form action="" method="post">
						...
						<?php
							...
							Token::create();
						?>
						<input type="submit" class="button" value="Change name" class="btn btn-info">
			</form></td>
			...
			<td><form action="" method="post">
						...
						<?php
							...
							Token::create();
						?>
						<input type="submit" value="Delete Character" class="button needconfirmation">
			</form>
